...
39. En alzas_produccion, la columna "Observaciones" NO debe recibir numeros que visualmente pertenecen a Tujo (gr), Mina (gr) o Bs. Si un numero esta bajo una columna de gramos, ponlo en esa columna. Si esta bajo Bs, ponlo en monto_ingreso.
40. Si no puedes ubicar con seguridad un numero entre Tujo (gr), Mina (gr) o Bs, no lo pegues en observaciones como texto normal. Ponlo en "valores_sin_ubicar" y marca "campos_dudosos".
41. Observaciones es solo para texto escrito en la columna Observaciones, notas como "Fundido", tachaduras, aclaraciones o dudas de lectura. No uses Observaciones como basurero de numeros.
...
export function crearPromptAnalisisEstructuraTabla({ tipoFuente = "auto", orientacion = "normal" } = {}) {
  return `Actua como analista documental experto en cuadernos manuscritos de cooperativas mineras auriferas de Tipuani, Bolivia.

Tu tarea NO es extraer filas todavia. Tu tarea es construir el MAPA DE LA TABLA para que otra lectura posterior pueda extraer correctamente.

Contexto solicitado: ${tipoFuente}.

${
  orientacion === "auto"
    ? `El sistema puede enviarte cada pagina en varias versiones de orientacion. Para cada pagina, el orden es: original, girada_180, rotar_90_derecha, rotar_90_izquierda. No son paginas diferentes. Elige la version donde la cabecera y las columnas se lean mejor y devuelve ese valor exacto en "version_elegida".`
    : `La imagen ya fue enviada con la orientacion seleccionada por el usuario: ${orientacion}.`
}

Reglas:
1. Primero identifica orientacion, cabecera, bordes de tabla y direccion de lectura.
2. Enumera solo columnas reales visibles. No agregues columnas por costumbre.
3. Si hay una columna de numeracion como N, No, Nro o Fila, descartala como columna editable y usala solo como referencia visual.
4. Si el nombre del area es encabezado de columna, por ejemplo Tujo (gr), Mina (gr), Rio (gr), manten cada area como columna propia.
5. Detecta el tipo de dato de cada columna: fecha, dinero_bolivianos, peso_gramos, cantidad, texto, recibo, folio, saldo, persona, item, observacion.
6. Observaciones solo es texto escrito en esa columna. No la uses para guardar numeros que pertenecen a otras columnas.
7. Identifica si hay totales, subtotales o notas fuera de filas normales.
8. Para cualquier formato nuevo, conserva las columnas tal como aparecen y asigna keys tecnicas estables en snake_case.
...
Devuelve SOLO JSON valido:
{
  ...
  "analisis_tabla": {
    "orientacion_imagen": "derecha | girada | de_lado | cabeza_abajo | dudosa",
    "version_elegida": "original | girada_180 | rotar_90_derecha | rotar_90_izquierda | unica | dudosa",
    "cabecera_detectada": "texto visible de cabecera",
    "direccion_lectura": "izquierda_a_derecha | derecha_a_izquierda | arriba_abajo | dudosa",
    "filas_visibles_estimadas": 0,
    "totales_detectados": "",
    "criterio_extraccion": "explicacion breve",
    "riesgos_lectura": []
  },
  "columnas_detectadas": [
    {
  ...
  "columnas_descartadas": [
    { "original": "No", "motivo": "numeracion visual" }
  ],
  "observaciones_pagina": ""
}`;
}

export function crearPromptExtraccionConEstructura({
  tipoFuente = "auto",
  orientacion = "normal",
  estructura,
} = {}) {
  return `Actua como extractor de datos manuscritos para auditoria de una cooperativa minera aurifera.

Ya existe un MAPA DE TABLA aprobado por el analisis previo. Tu tarea es extraer filas usando estrictamente ese mapa.

Contexto solicitado: ${tipoFuente}.

${
  orientacion === "auto"
    ? `Usa la version_elegida indicada en el mapa y NO dupliques filas.`
    : `Usa la orientacion ya seleccionada por el sistema o por el usuario: ${orientacion}.`
}
...
Reglas estrictas:
1. Usa SOLO las columnas de "columnas_detectadas" cuyo campo "descartada" no sea true.
2. No cambies nombres de columnas, keys ni tipo de dato.
3. Extrae cada valor segun su columna horizontal real. No muevas datos a Observaciones por duda.
4. Si un numero no se puede ubicar con seguridad en su columna, ponlo en "valores_sin_ubicar" y marca campos_dudosos.
5. Observaciones solo puede contener texto escrito en la columna Observaciones.
6. No inventes fechas, montos, pesos, recibos ni folios.
7. No incluyas encabezados, subtitulos, totales ni notas como filas.
8. Mantén el orden visual de las filas.
9. Normaliza Bs como numeros decimales en bolivianos.
10. Normaliza gr/g/gramos como numeros de peso en gramos.
11. Si el lote es alzas_produccion, prioriza columnas de area/peso como Tujo (gr), Mina (gr), Rio (gr) sobre montos Bs.
12. Si una fila solo tiene fecha pero no tiene valores ni texto de movimiento, no la incluyas.
...
Devuelve SOLO JSON valido con este formato:
{
  ...
  "analisis_tabla": ${JSON.stringify(estructura?.analisis_tabla || {}, null, 2)},
  "columnas_detectadas": ${JSON.stringify(estructura?.columnas_detectadas || [], null, 2)},
  "resumen": {
    "cantidad_filas_detectadas": 0,
    "total_montos_detectado": 0,
    "hay_total_en_imagen": false,
...
export function crearPromptAuditoriaExtraccionTabla({
  tipoFuente = "auto",
  estructura,
  extraccion,
} = {}) {
  return `Actua como auditor final de extraccion manuscrita para una cooperativa minera aurifera.

Tu tarea es comparar la imagen contra el JSON extraido y corregir errores ANTES de que los datos pasen al sistema.

Contexto solicitado: ${tipoFuente}.

MAPA DE TABLA CONFIRMADO:
${JSON.stringify(estructura, null, 2)}
...
Reglas de auditoria:
1. No des por bueno el JSON previo. Revisa visualmente columna por columna y fila por fila.
2. Si un valor esta bajo una columna en la imagen, debe quedar en esa misma columna/key del JSON.
3. Corrige desplazamientos horizontales: por ejemplo, un valor escrito bajo "Tujo (gr)" no puede quedar en "Mina (gr)" ni en "Bs".
4. Corrige desplazamientos verticales: una fecha y sus valores deben pertenecer a la misma linea visual.
5. Si el encabezado muestra dos columnas "Bs" pero no hay valores por fila, no inventes importes por fila. Los totales van a resumen u observaciones_pagina.
6. Si un valor no se puede ubicar con seguridad, ponlo en "valores_sin_ubicar" y marca campos_dudosos. No lo escondas en observaciones.
7. Observaciones solo contiene texto visible en la columna Observaciones.
8. No agregues filas que no existan. No incluyas totales como movimiento normal.
9. No elimines una fila real solo porque algun valor este vacio.
10. Si detectas que la extraccion previa esta muy dudosa, reduce confianza y explica el problema en observaciones_pagina.
11. Devuelve las columnas detectadas del mapa confirmado. No cambies la estructura salvo que una columna marcada como editable sea claramente incorrecta.
12. Devuelve SOLO JSON valido.

Formato de respuesta:
{
  "tipo_lote_detectado": "${estructura?.tipo_lote_detectado || extraccion?.tipo_lote_detectado || tipoFuente}",
  "tipo_documento": "${tipoFuente}",
  "confianza_tipo_lote": "${estructura?.confianza_tipo_lote || extraccion?.confianza_tipo_lote || "media"}",
  "analisis_tabla": ${JSON.stringify(estructura?.analisis_tabla || extraccion?.analisis_tabla || {}, null, 2)},
  "columnas_detectadas": ${JSON.stringify(estructura?.columnas_detectadas || extraccion?.columnas_detectadas || [], null, 2)},
  "resumen": {
    "cantidad_filas_detectadas": 0,
    "total_montos_detectado": 0,
    "hay_total_en_imagen": false,
  ...
  "alertas": [
    { "tipo": "columna_corregida | fila_dudosa | valor_sin_ubicar | total_detectado | otro", "descripcion": "" }
  ],
  "auditoria_extraccion": {
    "resultado": "corregida | sin_cambios | dudosa",
